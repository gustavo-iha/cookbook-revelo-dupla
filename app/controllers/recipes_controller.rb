class RecipesController < ApplicationController
  before_action :authenticate_user!,
                only: %i[new create edit update pending approve reject]
  before_action :authorized_user_list, only: %i[add_to_list]
  before_action :set_recipe, only: %i[edit show update destroy add_to_list]
  ...
  def search
    ...
    flash[:notice] = "#{msg}#{params[:query]}" unless @recipes.any?
  end
  ...
  def authorized_user_list
    ...
    redirect_to root_path unless @recipe_list.owned? current_user
  end
